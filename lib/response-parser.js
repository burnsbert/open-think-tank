...
function isEngageActionType(actionType) {
	return typeof actionType === 'string' && /^engage_[a-z0-9_-]+$/i.test(actionType);
}
...
function parseActionType(obj) {
	...
	if (typeof provided === 'string') {
		...
		if (!VALID_ACTION_TYPES.has(trimmed) && !isEngageActionType(trimmed)) {
...
function validateActionTypeMapping(action, actionType) {
	...
	if (isEngageActionType(actionType)) {
		return action === 'speak'
			? { success: true }
			: { success: false, error: `actionType "${actionType}" must be paired with action "speak"` };
	}
